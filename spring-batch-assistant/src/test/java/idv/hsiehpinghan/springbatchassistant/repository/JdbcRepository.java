...
public class JdbcRepository {
	...
	public int insertByPreparedStatementCreator(JdbcEntity entity) {
		...
		return jdbcTemplate.update(new PreparedStatementCreator() {
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				String sql = "INSERT INTO spring_batch_assistant.jdbcentity(id, primativeboolean, primativebyte, primativedouble, primativefloat, primativeint, primativelong, primativeshort, string, bigdecimal, sqldate, sqltime, sqltimestamp, bytearray) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
	...
	public JdbcEntity queryForObjectByRowMapper(long id) {
		String sql = "SELECT id, primativeboolean, primativebyte, primativedouble, primativefloat, primativeint, primativelong, primativeshort, string, bigdecimal, sqldate, sqltime, sqltimestamp, bytearray FROM spring_batch_assistant.jdbcentity where id = ?;";
		Object[] args = new Object[] { id };
		return jdbcTemplate.queryForObject(sql, args, new RowMapper<JdbcEntity>() {
			public JdbcEntity mapRow(ResultSet rs, int paramInt) throws SQLException {
	...
	public int updateByPreparedStatementCreator(JdbcEntity entity) {
		...
		return jdbcTemplate.update(new PreparedStatementCreator() {
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				String sql = "UPDATE spring_batch_assistant.jdbcentity SET primativeboolean=?, primativebyte=?, primativedouble=?, primativefloat=?, primativeint=?, primativelong=?, primativeshort=?, string=?, bigdecimal=?, sqldate=?, sqltime=?, sqltimestamp=?, bytearray=? WHERE id=?";
				PreparedStatement ps = con.prepareStatement(sql);
	...
	public void deleteByPreparedStatementCreator(final Long id) {
		jdbcTemplate.update(new PreparedStatementCreator() {
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				String sql = "DELETE FROM spring_batch_assistant.jdbcentity WHERE id=?";
				PreparedStatement ps = con.prepareStatement(sql);
				ps.setLong(1, id);
				return ps;
			}
	...
	public boolean exists(long id) {
		String sql = "SELECT COUNT(1) FROM spring_batch_assistant.jdbcentity WHERE id=?;";
		Object[] args = new Object[] { id };

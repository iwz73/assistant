...
public class RolesResourceTest extends AbstractTestNGSpringContextTests {
	@Autowired
	...
	public void apiClusterListTest() throws Exception {
		...
		for (ApiCluster apiCluster : apiClusterList) {
			...
			for (ApiService apiService : apiServiceList) {
				String serviceName = apiService.getName();
				System.err.println("----------------" + serviceName + "----------------");
				RolesResourceV11 rolesResource = servicesResource.getRolesResource(serviceName);
				ApiRoleList apiRoleList = rolesResource.readRoles(DataView.FULL_WITH_HEALTH_CHECK_EXPLANATION);
				System.err.println(objectMapper.writeValueAsString(apiRoleList));
			}
		}
	}
}

 ...
 * are Gora objects, generated from
...
public class LogManager {
	...
	private void init() throws IOException {
		...
		dataStore = DataStoreFactory.getDataStore(Long.class, Gora.class,
				new Configuration());
	}
...
//	}
	...
	private void storePageview(long key, Gora pageview) throws IOException,
			Exception {
		log.info("Storing Gora in: " + dataStore.toString());
	...
	private void get(long key) throws IOException, Exception {
		Gora pageview = dataStore.get(key);
	...
	private void query(long key) throws IOException, Exception {
		...
		Result<Long, Gora> result = query.execute(); // Actually executes the
														// query.
	...
	private void query(long startKey, long endKey) throws IOException,
		...
		Result<Long, Gora> result = query.execute();
	...
	private void deleteByQuery(long startKey, long endKey) throws IOException,
			Exception {
		// Constructs a query from the dataStore. The matching rows to this
		...
		log.info("pageviews with keys between " + startKey + " and " + endKey
				+ " are deleted");
	}

	private void printResult(Result<Long, Gora> result) throws IOException,
			Exception {

		while (result.next()) { // advances the Result object and breaks if at
								// end
			long resultKey = result.getKey(); // obtain current key
			Gora resultPageview = result.get(); // obtain current value object
		...
		System.out.println("Number of pageviews from the query:"
				+ result.getOffset());
	}

	/** Pretty prints the pageview object to stdout */
	private void printPageview(Gora pageview) {
		if (pageview == null) {
	...
	public static void main(String[] args) throws Exception {
		...
		if ("-parse".equals(args[0])) {
			...
			manager.deleteByQuery(Long.parseLong(args[1]),
